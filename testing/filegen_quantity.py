...
def main():
    ...
    output_dir = os.path.join(output_dir, 'quantity')
    print(f"📁 Tworzę folder dla zestawu testów...")
    ensure_dir(output_dir)

    for count in FILE_COUNTS:
        print(f"📁 Tworzę folder dla testu z ilością {count}...")
        count_output_dir = os.path.join(output_dir, f"test_quantity_{count}")
        ensure_dir(count_output_dir)

        for i in range(count):
            ext = random.choice(FILE_EXTENSIONS)
            filename = f"file_{i:04d}_{FILE_SIZE_KB}KB{ext}"
            file_path = os.path.join(count_output_dir, filename)
            generate_file(file_path, FILE_SIZE_BYTES)
